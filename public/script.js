...
function changeCursor(e) {
  if (
    document.elementsFromPoint(e.pageX, e.pageY).includes(colourBox) &&
    !myCursor.classList.contains("pipet")
  ) {
    ...
    document.elementsFromPoint(e.pageX, e.pageY).includes(header) &&
    !myCursor.classList.contains("paintCan")
  ) {
...
function touchClick(e) {
  const { pageX, pageY } = e;
  if (document.elementsFromPoint(pageX, pageY).includes(red)) {
    (ctx.strokeStyle = "#FF355E"), (myCursor.classList = "cursor red pipet");
  } else if (document.elementsFromPoint(pageX, pageY).includes(blue)) {
    (ctx.strokeStyle = "#0047AB"), (myCursor.classList = "cursor blue pipet ");
  } else if (document.elementsFromPoint(pageX, pageY).includes(yellow)) {
    (ctx.strokeStyle = "#FFFF00"),
      (myCursor.classList = "cursor yellow pipet ");
  } else if (document.elementsFromPoint(pageX, pageY).includes(green)) {
    (ctx.strokeStyle = "#50C878"), (myCursor.classList = "cursor green pipet");
  } else if (document.elementsFromPoint(pageX, pageY).includes(pink)) {
    (ctx.strokeStyle = "#FFC0CB"), (myCursor.classList = "cursor pink pipet");
  } else if (document.elementsFromPoint(pageX, pageY).includes(orange)) {
    (ctx.strokeStyle = "#FFA500"), (myCursor.classList = "cursor orange pipet");
  } else if (document.elementsFromPoint(pageX, pageY).includes(purple)) {
    (ctx.strokeStyle = "#9F2B68"), (myCursor.classList = "cursor purple pipet");
  } else if (document.elementsFromPoint(pageX, pageY).includes(black)) {
    (ctx.strokeStyle = "#000000"), (myCursor.classList = "cursor black pipet");
  } else if (document.elementsFromPoint(pageX, pageY).includes(lightBlue)) {
    (ctx.strokeStyle = "#72BCD4"),
      (myCursor.classList = "cursor lightBlue pipet");
  } else if (document.elementsFromPoint(pageX, pageY).includes(header)) {
    header.classList = myCursor.classList[1];
    ...
    document.elementsFromPoint(e.pageX, e.pageY).includes(removePenWidth)
  ) {
  ...
  socket.emit("penClick", {
    userId: localUser,
    otherWidth: window.innerWidth,
    otherHeight: window.innerHeight,
    pageX: pageX,
    pageY: pageY,
    ctxColour: ctx.strokeStyle,
    penClassList: myCursor.classList,
...
function mouseMoving(event) {
  ...
  if (
    ...
    myCursor.setAttribute(
      "style",
      `width: ${styleWidth}px; height: ${styleHeight}px;top: ${
        pageY - styleHeight
      }px; left: ${pageX}px`
    );
  } else {
    myCursor.setAttribute(
      "style",
      `width: ${myCursor.style.width}; height: ${myCursor.style.height}; top: ${pageY}px; left: ${pageX}px`
    );
  }
  if (penDown) {
    ctx.lineTo(x, y);
...
function otherUserDrawing(userInfo) {
  ...
  if (userId !== localUser) {
    ...
    if (
      userPen.classList.contains("pipet") ||
      userPen.classList.contains("paintCan")
    ) {
      userPen.setAttribute(
        "style",
        `width: ${styleWidth}px; height: ${styleHeight}px;top: ${
          pageY + height - styleHeight
        }px; left: ${pageX + width}px`
      );
    } else {
      userPen.setAttribute(
        "style",
...
socket.on("pendrawing", function (userInfo) {
  if (
    !document.getElementById(`userPen-${userInfo.userId}`) &&
    userInfo.userId !== localUser
  ) {
